...
1. The dependency graph's true lower MSRV floor is unproven. The
   resolved set's declared minimums (ed25519-dalek 2.2.0 at 1.81,
   base64ct 1.8.3 at 1.85, serde_with 3.23.0 at 1.88) suggest a floor
   at or below 1.88 could exist, but no toolchain older than 1.96.0 is
...
- Observation: the `ci.yml` step list (capabilities, formatting,
  ...
  declared MSRV.
- Interpretation: a floor at or below 1.88 is plausible from the
  declared minimums, but plausible is not proven — compilation
  evidence for older toolchains does not exist (D-000016 forbids
  metadata-inferred floor claims).

## Impact

Consumers building on toolchains between roughly 1.81 and 1.96 have no
