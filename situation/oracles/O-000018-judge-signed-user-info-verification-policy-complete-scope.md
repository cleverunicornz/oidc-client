...
- P1 — The default UserInfo verifier and request-builder configuration accept
  only RS256; every other algorithm requires explicit `set_allowed_algs`
  configuration.
...
- P3 — An ES384 response with a matching P-384 key verifies when ES384 is
  explicitly allowed.
- P4 — An HS256 response verifies through a direct confidential verifier or a
  request builder configured with the matching client secret only when HS256
  is explicitly allowed.
- P5 — An HS384 response verifies through a direct confidential verifier or a
  request builder configured with the matching client secret only when HS384
  is explicitly allowed.
- P6 — An HS512 response verifies through a direct confidential verifier or a
  request builder configured with the matching client secret only when HS512
  is explicitly allowed.
...
- F1 — A default verifier or request builder accepts an ES256, ES384, or HS*
  response without an explicit allowlist entry.
- F2 — An ES256 or ES384 response signed by an altered or nonmatching key
  verifies.
- F3 — An HS256, HS384, or HS512 response verifies through a direct verifier
  or request builder with a missing or wrong client secret.
...
|---|---|---|
| P1 | Default direct and request-builder configuration is RS256-only; opt-in is required. | `src/verification/tests.rs::test_user_info_signed_response_es256`; `src/user_info.rs::test_user_info_request_signed_response_policy` |
...
| P4 | Allowed confidential HS256 verifies by direct and request-builder configuration. | `src/verification/tests.rs::test_user_info_signed_response_hs256`; `src/user_info.rs::test_user_info_request_signed_response_policy` |
| P5 | Allowed confidential HS384 verifies by direct and request-builder configuration. | manual — `src/core/jwk/mod.rs::CoreJsonWebKey::verify_signature` HS384 branch, shared-secret key resolution, and request-builder configuration forwarding |
| P6 | Allowed confidential HS512 verifies by direct and request-builder configuration. | manual — `src/core/jwk/mod.rs::CoreJsonWebKey::verify_signature` HS512 branch, shared-secret key resolution, and request-builder configuration forwarding |
...
| F1 | Default/non-allowlisted direct or request-builder algorithm acceptance is rejected. | `src/verification/tests.rs::test_user_info_signed_response_es256`; `src/user_info.rs::test_user_info_request_signed_response_policy`; manual for ES384/HS384/HS512 |
...
| F3 | Missing or wrong HS* secrets are rejected by direct and request-builder paths. | `src/verification/tests.rs::test_user_info_signed_response_hs256`; `src/user_info.rs::test_user_info_request_signed_response_policy`; manual for HS384/HS512 |
